...
print_header "FAIT Co-op Headless Cypress Testing with Reporting"

# Check if mochawesome dependencies are installed
if ! npm list mochawesome &>/dev/null || ! npm list mochawesome-merge &>/dev/null || ! npm list mochawesome-report-generator &>/dev/null; then
  echo -e "${YELLOW}Installing Mochawesome reporting dependencies...${NC}"
...
# Create a temporary cypress.json file with reporter options
echo '{
...
for category in $TEST_SPECS; do
  ...
  # Run the tests in headless mode with Mochawesome reporter
...
echo -e "${YELLOW}Merging test results...${NC}"
...
if [ "$ALL_PASSED" = true ]; then
  ...
  echo -e "${YELLOW}Test report is available at: cypress/results/report.html${NC}"
  exit 0
else
  echo -e "${RED}${BOLD}Some tests failed. Check the test report for details.${NC}"
  echo -e "${YELLOW}Test report is available at: cypress/results/report.html${NC}"
  exit 1
fi
