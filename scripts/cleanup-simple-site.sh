...
if [ -n "$OLD_IMAGES" ]; then
  ...
  if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo "Listing all images with tags..."
    gcloud container images list-tags gcr.io/$PROJECT_ID/$SIMPLE_SERVICE_NAME

    # Delete by tag instead
    echo "Would you like to delete all images with a specific tag? (y/n) "
    read -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
      read -p "Enter the tag to delete (e.g., latest): " TAG
      gcloud container images delete gcr.io/$PROJECT_ID/$SIMPLE_SERVICE_NAME:$TAG --quiet
    fi
