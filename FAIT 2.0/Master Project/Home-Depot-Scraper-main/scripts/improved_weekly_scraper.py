#!/usr/bin/env python3
"""
Improved Weekly Building Supplies Scraper
...
class BuildingSuppliesScraper:
    ...
    def _fetch_category_page(self, page):
        ...
        try:
            with urllib.request.urlopen(api_url) as response:
                ...
                with open(os.path.join(self.output_dir, "raw_data", f"page_{page}.json"), 'w') as f:
    ...
    def _extract_product_urls(self, page_data):
        ...
        for product in products:
            ...
            # Extract product URL and make sure it's a valid URL
            url = product.get('product', {}).get('link')
            if url and url.startswith('http'):
                self.product_urls.append(url)
                self.products_scraped += 1
                log(f"Extracted product URL: {url}")
...
class ProductDetailExtractor:
    ...
    def extract_products(self, product_urls):
        ...
        for i, url in enumerate(product_urls):
            if i % 10 == 0 or i == len(product_urls) - 1:
    ...
    def extract_product_info(self, url):
        ...
        try:
            with urllib.request.urlopen(api_url) as response:
                ...
                os.makedirs(os.path.join(self.output_dir, "raw_data", "products"), exist_ok=True)
                item_id = product.get('item_id', 'unknown')
                with open(os.path.join(self.output_dir, "raw_data", "products", f"product_{item_id}.json"), 'w') as f:
                    json.dump(data, f, indent=2)
                
                log(f"Successfully extracted details for: {result['product_name']}")
                return result
...
def main():
    ...
    if product_urls:
        ...
        log("No product URLs found to extract details from")
