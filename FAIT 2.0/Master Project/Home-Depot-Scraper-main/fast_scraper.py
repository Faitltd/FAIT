#!/usr/bin/env python3
"""
Enhanced Home Depot Scraper with optimizations for speed and avoiding detection.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"fast_scraper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Enhanced Home Depot Scraper")
    ...
    parser.add_argument('--min-delay', type=float, default=0.5, help='Minimum delay between API requests in seconds')
    parser.add_argument('--max-delay', type=float, default=2.0, help='Maximum delay between API requests in seconds')
    ...
    parser.add_argument('--backoff-factor', type=float, default=0.5, help='Backoff factor for retries')
...
def process_url(url, scraper, min_delay, max_delay, max_retries, backoff_factor):
    """Process a single URL and return the product data."""
    ...
    scraper.client.session.headers.update({'User-Agent': get_random_user_agent()})

    # Try to fetch product details with retries and exponential backoff
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                logger.warning(f"Error processing URL {url}: {str(e)}. Retrying in {backoff_time:.2f} seconds...")
                time.sleep(backoff_time)
            else:
                logger.error(f"Error processing URL {url} after {max_retries} retries: {str(e)}")
                return None

    return None

def process_batch(urls, scraper, min_delay, max_delay, max_retries, backoff_factor):
    """Process a batch of URLs in parallel and return the product data."""
    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(urls)) as executor:
        # Submit all URLs for processing
        future_to_url = {
            executor.submit(
                process_url, url, scraper, min_delay, max_delay, max_retries, backoff_factor
            ): url for url in urls
        }
    ...
    # Add a random delay between batches to avoid detection
...
def main():
    ...
    logger.info("Starting Enhanced Home Depot Scraper")
    ...
    try:
        ...
        batch_size = min(args.batch_size, len(urls))
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            # Update progress
            update_progress(processed_count, len(urls), f"Processing batch {batch_idx+1}/{len(batches)}")
            logger.info(f"Processing batch {batch_idx+1}/{len(batches)} ({len(batch)} URLs)")
            ...
            # Add results to the scraper's products list
            scraper.products.extend(batch_results)
            ...
            if args.auto_save_interval > 0 and len(scraper.products) >= last_save_count + args.auto_save_interval:
                update_progress(processed_count, len(urls), f"Auto-saving {len(scraper.products)} products...")
                save_partial_results(scraper, args.output_dir, "autosave")
                last_save_count = len(scraper.products)
                update_progress(processed_count, len(urls), f"Continuing with batch {batch_idx+2}/{len(batches)}")

            # Update progress after processing this batch
            update_progress(processed_count, len(urls), f"Processed {processed_count}/{len(urls)} URLs")
