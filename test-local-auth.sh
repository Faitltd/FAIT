...
echo -e "${GREEN}=== FAIT Co-op Local Authentication Test Runner ===${NC}"
echo ""
...
# Run only the local authentication test
echo -e "${YELLOW}Running local authentication test...${NC}"
npx cypress run --spec "cypress/e2e/local-auth-login.cy.js"
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
  echo -e "${GREEN}Local authentication tests passed!${NC}"
  echo -e "${GREEN}The login credentials are working correctly with local authentication.${NC}"
else
  echo -e "${RED}Local authentication tests failed.${NC}"
