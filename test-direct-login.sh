...
echo -e "${GREEN}=== FAIT Co-op Direct Login Test Runner ===${NC}"
echo ""
...
# Run only the direct login test
echo -e "${YELLOW}Running direct login test...${NC}"
npx cypress run --spec "cypress/e2e/direct-login-test.cy.js"
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
  echo -e "${GREEN}Direct login tests passed!${NC}"
  echo -e "${GREEN}The login credentials are working correctly with direct login.${NC}"
else
  echo -e "${RED}Direct login tests failed.${NC}"
