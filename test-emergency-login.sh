...
echo -e "${GREEN}=== FAIT Co-op Emergency Login Test Runner ===${NC}"
echo ""
...
# Run only the emergency login test
echo -e "${YELLOW}Running emergency login test...${NC}"
npx cypress run --spec "cypress/e2e/emergency-login-test.cy.js"
...
if [ $TEST_EXIT_CODE -eq 0 ]; then
  echo -e "${GREEN}Emergency login tests passed!${NC}"
  echo -e "${GREEN}The login credentials are working correctly with emergency login.${NC}"
else
  echo -e "${RED}Emergency login tests failed.${NC}"
