...
describe('Local Authentication Login Test', () => {
  ...
  testCredentials.forEach((cred) => {
    it(`should login successfully with ${cred.type} credentials using local authentication`, () => {
      // Log the test being run
      cy.log(`Testing login with ${cred.type} credentials: ${cred.email}`);
      
      // Take screenshot before login
      cy.screenshot(`${cred.type.toLowerCase()}-before-login-local-auth`);
      ...
      cy.screenshot(`${cred.type.toLowerCase()}-after-login-local-auth`);
