...
-- Function to safely create a trigger if it doesn't exist
...
-- Create trigger for updating updated_at using the safe function
...
-- Create trigger for updating updated_at using the safe function
SELECT safe_create_trigger(
  'update_service_agent_portfolio_items_updated_at',
...
-- Create trigger for updating updated_at using the safe function
SELECT safe_create_trigger(
  'update_service_agent_work_history_updated_at',
...
-- Create trigger for updating updated_at using the safe function
SELECT safe_create_trigger(
  'update_service_agent_references_updated_at',
...
-- Create trigger for new message notifications safely
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    AND tgrelid = (SELECT oid FROM pg_class WHERE relname = 'messages')
  ) THEN
...
-- Create trigger for warranty claim status change notifications safely
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'notify_warranty_status_change_trigger'
    AND tgrelid = (SELECT oid FROM pg_class WHERE relname = 'warranty_claims')
  ) THEN
