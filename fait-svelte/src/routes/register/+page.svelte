<script lang="ts">
  ...
  async function handleRegister() {
    ...
    if (password !== confirmPassword) {
      auth.clearError();
      setTimeout(() => {
        auth.updateProfile({ error: 'Passwords do not match' });
      }, 0);
      return;
    }
    ...
    if (!termsAccepted) {
      auth.clearError();
      setTimeout(() => {
        auth.updateProfile({ error: 'You must accept the terms and conditions' });
      }, 0);
      return;
    }
    ...
    <div class="max-w-md mx-auto">
      ...
      <Card variant="elevated" padding="lg">
        ...
        <form on:submit|preventDefault={handleRegister} class="space-y-4">
          ...
          <Button type="submit" variant="primary" fullWidth={true} disabled={$auth.isLoading}>
