<script lang="ts">
	...
	async function handleSocialLogin(provider: string) {
		...
		try {
			let result;
			if (provider === 'Google') {
				result = await authActions.loginWithGoogle();
			} else if (provider === 'Facebook') {
				result = await authActions.loginWithFacebook();
			} else {
				throw new Error(`${provider} login not implemented yet`);
			}

			if (!result.success) {
				errorMessage = result.error || `Failed to login with ${provider}`;
			}
		...
		<div class="bg-white rounded-2xl shadow-xl p-8">
			...
			<div class="mt-6 grid grid-cols-2 gap-3">
				<button
					...
					disabled={isSubmitting}
					class="w-full inline-flex justify-center py-3 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 transition-all transform hover:scale-105 {isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}"
				>
