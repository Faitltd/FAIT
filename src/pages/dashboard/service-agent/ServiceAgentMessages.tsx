...
const ServiceAgentMessages = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        // Get service packages for this service agent
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        console.log('Fetching messages for booking ID:', selectedBookingId);

        // Fetch messages from the database with a more detailed query
    ...
    console.log('Setting up real-time subscription for booking ID:', selectedBookingId);
    const channelName = `messages-${selectedBookingId}`;
      ...
      .on(
        'postgres_changes',
        {
          ...
          try {
            ...
            console.log('Formatted new message:', newMsg);

            // Update both the current messages and stored messages
            setMessages(prevMessages => {
              const updatedMessages = [...prevMessages, newMsg];

              // Also update the stored messages
              setMessagesByBookingId(prev => ({
                ...prev,
    ...
    console.log('Subscription created for channel:', channelName);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Get the client ID from the selected booking
      const selectedBooking = bookings.find(b => b.id === selectedBookingId);
      if (!selectedBooking) throw new Error('Booking not found');

      const clientId = selectedBooking.client_id;
      ...
      // but we'll add it manually as a fallback
      const newMessageObj = {
        id: messageData.id,
